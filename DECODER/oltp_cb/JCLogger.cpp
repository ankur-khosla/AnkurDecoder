...
LogUnit::LogUnit(int iBlkSize, int iUnitSize)
{
	m_iBlkSize=iBlkSize;
	m_iUnitSize=iUnitSize;
	if ((m_iBlkSize*m_iUnitSize) <= 0)
	{
		m_iBlkSize=BLOCK_SIZE;
		m_iUnitSize=LOG_UNIT_SIZE;
	}
	m_iBufSize=m_iBlkSize*m_iUnitSize;
	m_cpBuf=new unsigned char[m_iBufSize];
	m_iCurrentIdx=0;
}
...
//	short msgSize = 0;
//
//	//memcpy(&msgSize, binArr + (25 + exLen), 2);//get message size: first 2 bytes of one whole message
//	//memcpy(m_pMsg->m_cpBuf, binArr + (25 + exLen), msgSize); //copy from the specified index: payload header size (27) + extra data length.
//
//	//m_pMsg->m_cpBuf = binArr;
//	memcpy(m_pMsg->m_cpBuf,binArr,174);
//
//	struct LOG *pMlog;
//	pMlog = (struct LOG *)m_pMsg->m_cpBuf;
//
//
...
//
//	struct tm *B_day;
//	B_day = localtime(&pMlog->hdr.timelu);
//
...
short JCLogger::GetSysNo()
{
	return m_pCtrlBlk->m_iSysNo;
}
