...
class DeSelMap : public CBMsgTranslator  
{
public:
	char* GetType	(char m_cType, char Type[10]);

	void GetSel		(const struct LOG *pMlog, unsigned char m_cType, char bufs[1024]);
	
	void fmtAup		(const struct LOG * pMlog, char sels[1024]);
	
	void fmtMk6		(const struct LOG * pMlog, unsigned int m_iType, char sels[300]);
	
	void fmtNrm		(const struct LOG *pMlog, char sels[1024]);
	
	void fmtInd		(const struct LOG *pMlog, const bool allupt, // true if allup
						const unsigned short idwu, // id no. for allup array
						char sels[200]); // selections
	
	void fmtQin		(const struct LOG *pMlog, const unsigned short numofbnk, // num of bankers
	...
	void fmtQin2( const struct LOG *pMlog, const unsigned short numofbnk, // num of bankers
						 const unsigned short numofbmp, // num of bitmaps
						 const unsigned short bmpposwu, // bitmap position
						 const bool allupt, // true if allup
						 const unsigned short idwu, // id no. of allup array
	...
	void fmtSln		(const struct LOG *pMlog, const unsigned short bmppos, // bitmap position 
						const bool allupt, // true if allup
						const unsigned short idwu, // id no. for allup array
						char sels[]); // selections

	void fmtSln2( const struct LOG *pMlog, const unsigned short bmppos, // bitmap position 
						 const bool allupt, // true if allup
						 const unsigned short idwu, // id no. for allup array
						 char sels[]); // selections
	
	void fmtExt		(const struct LOG *pMlog, const unsigned short numofbmp, // number of bitmap
						const unsigned short numofbnk, // number of banker
						char sels[]); // selections

	void fmtExtAup(const struct LOG *pMlog, const unsigned short numofbmp, // number of bitmap
		const unsigned short idwu, // id no. of allup array
		char sels[]); // selections
